...
//Handlebars.registerPartial('part_bar', jui2.tmpl.tmpl_part_bar);
;/****js/lang.monthPicker.js****/
(function($){
...
(function($){
	...
	var table = function(opt){
		...
		if(target.hasClass('j-body')){
			...
			output.children('.j-table-preview').on('dblclick doubletap', function(e){
				e.preventDefault();
				$(this).empty();
			})
	...
	$widget.fn.resizeTable = function(){
		...
		$(self[0]).children('.j-table-head').find('thead tr:nth-child(1) td').each(function(i, val){
			$(self[0]).children('.j-table-body').find('table colgroup:nth-child('+(i+1)+') col').width($(val).outerWidth(false))
		})
	}
	...
	$widget.fn.generateData = function(data){
		...
		if(data){
			...
			$jTableBody.children().children().children('tr').children('td').on('dblclick doubletap', function(e){
				e.preventDefault();
				var $el = $(this), index = $(this).index(), $parent = $el.parent();
				if(defaults.editable[index]){
					$parent[0].defaultInnerHTML = $parent.children().clone(!0);
			...
			function draw(data){
				if(param.sEcho==data.sEcho){
					...
					$self.children('.j-table-body > table:nth-child(1)').width($self.children('.j-table-head').outerWidth(false))
			...
			if(defaults.initial && self.param.sEcho == 0){
				param.iDisplayLength = defaults.initial.iTotalDisplayRecords
				draw(defaults.initial)
				self.loadingMask.destroy()
			}
			else{
	...
	$widget.fn.settings = function(){
		...
		if(0 in table.children('.j-table-body').children('.j-table-settings')){
			...
			table.children('.j-table-body').children().velocity("transition.slideUpIn", {complete: function(){
				...
				table.find('.j-table-body table.j-body').outerWidth(table.find('.j-table-head table').outerWidth(false))
			}});
		}
		else{
			var column = '';
	...
	$(window).scroll(function(){
		$('[role="table"] > .j-table-body').each(function(i, val){
			var el = $(this);
			...
			if(jui2.data.followHeadShow != false){
				if( (el.offset().top-$(window).scrollTop()) < jui2.data.followHeadShow && !jui2.isInView(el.next())){
					...
					if( (el.next().offset().top-$(window).scrollTop()) < jui2.data.followHeadShow || (el.prev().prev().offset().top-$(window).scrollTop()) > jui2.data.followHeadShow){
						var head = el.prev();
						head.css({
							'width': head.prev().outerWidth(false),
							top: '',
							position: 'relative',
							'z-index': '',
							'overflow-x': 'hidden'
...
;/****js/widget/table.plugin.export.js****/
	(function($){
		...
		$(selector+'table[jui2=true][role=table]').each(function(){
			...
			$.each(this.attributes, function(i, val){
				...
				data[val.name] = ['drop', 'server', 'jui2', 'hidden', 'merge', 'stickycolumn'].indexOf(val.name) > -1 ?
					eval(val.value)
				: ['event', 'bottombar', 'topbar', 'custom', 'preview', 'editable', 'save', 'param', 'initial'].indexOf(val.name) > -1 ?
					vr[val.value]
				:
					val.value
				/*
