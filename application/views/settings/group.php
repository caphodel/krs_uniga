...
<script>
	$('#btnAddGroup').click(function(){
		$.ajax({
			url: 'index.php/settings/add_group',
	...
	$('#btnDeleteGroup').click(function(){
		if(confirm("Apakah Anda yakin akan menghapus group "+$('#list_group').val()[0][1]+"?")){
			$.getJSON('index.php/settings/delete_group', {'user_group_id': $('#list_group').val()[0][0]}).success(function(){
	...
	$('#btnEditGroup').click(function(){
		$.ajax({
			url: 'index.php/settings/edit_group',
			success: function(data){
				$('#settings').append(data);
			}
		})
