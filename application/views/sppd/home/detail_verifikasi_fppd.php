					...
					<td width="20%">Tujuan</td><td width="1%">:</td><td colspan="4">
						<j-table width="600px" id="list_tujuan">
							<j-toolbar id="toolbar">
									<j-spacer></j-spacer><j-button color="blue" id="btnEditTujuan">Edit</j-button>
							</j-toolbar>
							  [
								["No", "Kota", "Kategori"]	
							  ]
		...
		<table style="border-collapse: collapse;border: 1px solid #FFFFFF;" width="100%" id="table_tindakan_karyawan">
			<tr height="40px" id="tr_list_biaya">
				<td width="20%">Lain Lain</td><td width="1%">:</td><td colspan="4">
					<j-radiofield id="is_pu" onclick="handleClick(this);">[["PU", "Pembayaran Oleh PU"], ["KEU", "Pembayaran Oleh keuangan"]]</j-radiofield>
					<j-table width="600px" id="list_biaya">
							<j-toolbar id="toolbar">
								<j-spacer></j-spacer><j-button color="blue" id="btnAddBiaya">Add</j-button>
							</j-toolbar>	
						  [
			...
			<tr height="40px">
				<td width="20%">Keterangan Verifikasi</td><td width="1%">:</td><td colspan="4"><j-textArea id="fppd_note_verifikasi"></j-textArea></td>
			</tr>					
			<tr height="40px">
...
<script>
	...
	$('#btnEditTujuan').click(function(){
		if ($('#list_tujuan')[0].selectedIndex.length==0){
			...
			if ($('#list_tujuan')[0].selectedIndex.length==1){
				$.ajax({
					url: 'index.php/sppd_home/pop_up_edit_tujuan',
					data : {
						fppd_id : '<?php echo $_REQUEST['fppd_id']; ?>',
						idx : $('#list_tujuan')[0].selectedIndex[0],
						user : user,
						destination_id : $('#list_tujuan')[0].record[$('#list_tujuan')[0].selectedIndex[0]][3],
	...
	$('#btnAddBiaya').click(function(){
		$.ajax({
			url: 'index.php/sppd_home/pop_up_add_biaya',
			data : {
				fppd_id : '<?php echo $_REQUEST['fppd_id']; ?>'
			},
			success: function(data){
	...
	function handleClick(radio){
		var fppd_id='<?php echo $_REQUEST['fppd_id']; ?>'
		var val = $('#is_pu').val()
		var ket = $('#fppd_note_verifikasi').val()
		if (val=='PU'){
			if (check_is_pu(fppd_id)){
				array_is_pu.push([fppd_id, val, ket])
			}else{
				update_is_pu(fppd_id, val, ket)
			}
		}else if (val=='KEU'){
			if (check_is_pu(fppd_id)){
				array_is_pu.push([fppd_id, val, ket])
	 ...
	 $('#fppd_note_verifikasi').keyup(function(){
		var fppd_id='<?php echo $_REQUEST['fppd_id']; ?>'
		var val = $('#is_pu').val()
		var ket = $('#fppd_note_verifikasi').val()
	...
	check_last_update('<?php echo $_REQUEST['fppd_id']; ?>')
	
	function check_last_update(fppd_id){
		if (check_is_pu(fppd_id)==false){
			$('#is_pu').val(select_is_pu(fppd_id, 1))
			$('#fppd_note_verifikasi').val(select_is_pu(fppd_id, 2))
		}
	}
