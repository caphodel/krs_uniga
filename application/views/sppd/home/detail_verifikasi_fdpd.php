...
<div id="div_add_fdpd" class="j-panel">
	...
	<div id="div_tindakan" class="j-panel" data-content="TINDAKAN">
			...
			<tr height="40px">
				<td width="20%">Keterangan  Verifikasi</td><td width="1%">:</td><td colspan="4"><j-textArea id="fdpd_note_verifikasi"></j-textArea></td>
			</tr>
			<tr height="40px">
...
<script>
	...
	$('#btnEditTujuan').click(function(){
		if ($('#list_tujuan')[0].selectedIndex.length==0){
			...
			if ($('#list_tujuan')[0].selectedIndex.length==1){
				$.ajax({
					url: 'index.php/sppd_home/pop_up_edit_tujuan_fdpd',
					data : {
						fdpd_id : '<?php echo $_REQUEST['fdpd_id']; ?>',
	...
	function count_kekurangan_biaya(arr){
	 ...
	 $('#fdpd_note_verifikasi').keyup(function(){
		var fdpd_id='<?php echo $_REQUEST['fdpd_id']; ?>'
		var ket = $('#fdpd_note_verifikasi').val()
		
		if (check_is_pu(fdpd_id)){
			array_is_pu.push([fdpd_id, ket])
		}else{
			update_is_pu(fdpd_id, ket)
		}
		
    });
	
	check_last_update('<?php echo $_REQUEST['fdpd_id']; ?>')
	
	function check_last_update(fdpd_id){
		if (check_is_pu(fdpd_id)==false){
			$('#fdpd_note_verifikasi').val(select_is_pu(fdpd_id, 1))
		}
	}
