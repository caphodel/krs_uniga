<div id="div_batal_keu">
	<j-table id="list_konfirmasi_batal_keu" class="doc" title="List Konfirmasi Batal Keuangan">
		...
		<j-loader src="index.php/sppd_home/get_data_fppd_batal_keu?user=<?php echo $_REQUEST['user']; ?>"></j-loader>
...
<script>
	...
	$('#btnKonfirmasi').click(function(){
		if($('[name=select]:checked').val()){
			if(confirm("Apa anda yakin ?")){
				...
				$.getJSON('index.php/sppd_home/validasi_fppd_batal_keu', {
					user : user,
					fppd_id : fppd_id
				}).done(function(json){
					if (json.STATUS==1){
						alert(''+json.MESSAGE+'');
						$('#list_konfirmasi_batal_keu')[0].generateData();
					}
				})
			}
		}else{
	...
	$('#list_konfirmasi_batal_keu').on('afterdraw', function(){
		reload_badge()
	})
	
	$('#btnSearch').click(function(){
		$('#list_konfirmasi_batal_keu j-loader')[0].param.sSearch=$('#tfSearch').val()
		$('#list_konfirmasi_batal_keu')[0].generateData()
	})
