...
<div id="div_add_fppd" class="j-panel">
	...
	<div id="div_tindakan" class="j-panel" data-content="TINDAKAN">
			...
			<table style="border-collapse: collapse;border: 1px solid #FFFFFF;" width="100%" id="table_perjalanan_dinas">
				<tr height="40px">
					<td width="20%">Pembatalan</td><td width="20%" colspan="3"> : <a id="cancel_note"></a></td>
				</tr>
				<tr height="40px">
					<td width="20%">Perpanjangan/Percepatan</td><td width="20%" colspan="3"> : <a id="extend"></a></td>
				</tr>
				<tr height="40px">
...
<script>
	...
	function load_det(id){
		...
		//select Cancel
		$.getJSON('index.php/sppd_fppd_list_detail/get_data_pembatalan', {
			fppd_id : fppd_id
		}).done(function(jsn){
			if (jsn.aaData!=null){
				$.each(jsn.aaData, function(i,val){
					$('#cancel_note').text(jsn.aaData[i][0])
				})
			}
		})
		
		//select Perpanjangan
		$.getJSON('index.php/sppd_fppd_list_detail/get_data_perpanjangan', {
			fppd_id : fppd_id
		}).done(function(jsn){
			if (jsn.aaData!=null){
				$.each(jsn.aaData, function(i,val){
					$('#extend').text(jsn.aaData[i][0])
				})
			}
		})
